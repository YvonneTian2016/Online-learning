...
c_num = k;
[off_sq_sample,off_sq] = Sample2D_Stratified(gI,k,u,v,u_cdf,v_cdf);
off_sq = off_sq';
...
while(converged == 0)    
     ...
     % [sq,pdf_sigma] =Pic_Sam(gI,1,cdf);
      [onli_sq_sample,sq] = Sample2D_test(gI,1,u,v,u_cdf,v_cdf); 
...
imshow(image);
save('Figs/imageK100M10000.mat','image');

%ConvergeFig=figure(4);
%plot(condition(:,1),condition(:,2),'r');
%line(condition(:,1),condition(:,2),'Marker','o');

savefig(GaussFig,'Figs/GaussK100M10000.fig');
savefig(ResultFig,'Figs/ResultK100M10000.fig');
