 ...
 pdf = zeros(q, k);
  ...
  for j = 1:k
    ...
    % U3(:,2*j-1:2*j,i) = eta*W(q,j)*(off_sq(:,q)*off_sq(:,q)'); 
     U3(i,j) = eta*W(q,j)*(off_sq(:,q)'*off_sq(:,q)); %ss^T->s^Ts
  end
...
while(converged == 0)    
    ...
    for j = 1:k
       ...
       %ss^T->s^Ts
       U3(i,j) = (1-eta)*U3(i-1,j)+eta*W(q,j)*(off_sq(:,q)'*off_sq(:,q));
    end
    ...
    for (j = 1 : k)
      ...
      %  A = U2(:,j,i)*mu(:,j)'+mu(:,j)*U2(:,j,i)'; %ST
       ...
       sigma(:,:,j)= (((b/n).*I + U3(i,j)-A+U1(i,j)*B)/((a-2)/n+Dim*U1(i,j)))*eye(2);
...
while(converged == 0)    
    ...
    for j = 1:k
       ...
       U3(i,j) = (1-eta)*U3(i-1,j)+eta*W(q,j)*(onli_sq(:,q)'*onli_sq(:,q));
    end
...
imshow(image);
save('Figs/imagek3m4_2.mat','image');

savefig(GaussFig,'Figs/k3m4Gauss_2.fig');
savefig(ResultFig,'Figs/k3m4Reuslt_2.fig');
savefig(imshow,'Figs/k3m4image_2.fig');