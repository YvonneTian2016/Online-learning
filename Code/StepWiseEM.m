...
plot(X(:, 1), X(:, 2),'b.');
hold on;
plot(mu1(1), mu1(2), 'kx', 'MarkerSize', 8, 'LineWidth', 6);
plot(mu2(1), mu2(2), 'kx', 'MarkerSize', 8, 'LineWidth', 6);
%plot(X2(:, 1), X2(:, 2), 'ro');
...
k = size(mu,1);  % The number of clusters.
...
for (q = 1 : N)
    ...
    for (j = 1 : k)
        ...
        mu(j, :) = (1-eta)*muold + eta*W(i,j)*Y(i,j);
    ...
    if(mod(i,5) == 0)
        ...
        colors = {'rx', 'go', 'b^', 'k<'};
        plot(mu(1,1), mu(1,2), char(colors( mod(i, 4)+1 )), 'MarkerSize', 8, 'LineWidth', 6);
        plot(mu(2,1), mu(2,2), char(colors( mod(i, 4)+1 )), 'MarkerSize', 8, 'LineWidth', 6);
