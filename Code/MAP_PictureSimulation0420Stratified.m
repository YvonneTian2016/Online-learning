...
[off_sq_sample,off_sq,sample_num] = Sample2D_Stratified(gI,N,u,v,u_cdf,v_cdf);
off_sq = off_sq';
...
while(converged == 0)    
    ...
    %axis normal;
   % hold on;
    condition(i) = abs(likhood/prevLikhood-1);
    
    %hold off;
    if (condition(i)<0.1)|| i > 10*N
           converged = 1;
    end
...
while(converged == 0)    
     ...
     % [sq,pdf_sigma] =Pic_Sam(gI,1,cdf);
      [onli_sq_sample,sq,sample_num] = Sample2D_Stratified(gI,k,u,v,u_cdf,v_cdf); 
...
ResultFig = Figure(5);
imshow((tim/sumpb)*(sum_int/255));


%ConvergeFig=figure(4);
%axis normal;
%scatter(1:m:i,condition(1:i));
%line(i:m:i,condition(1:i),'Marker','o');

savefig(PointsFig,'Points.fig');
savefig(GaussFig,'Gauss.fig');
%savefig(ConvergeFig,'Converge.fig');
savefig(ResultFig,'Reuslt.fig');
savefig(imshow,'image.fig');