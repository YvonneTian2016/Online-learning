...
while(converged == 0)    
    ...
    for(j = 1:k)
         sigma_scalar = max(sigma(1,1),sigma(2,2));
         sigma(:,:,j) = [sigma_scalar 0; 0 sigma_scalar];
    end
...
imshow(image);
save('Figs/sigma_scalar_imagek3m4.mat','image');

savefig(GaussFig,'Figs/sigma_scalar_k3m4Gauss.fig');
savefig(ResultFig,'Figs/sigma_scalar_k3m4Reuslt.fig');
savefig(imshow,'Figs/sigma_scalar_k3m4image.fig');
