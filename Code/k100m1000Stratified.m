...
c_num = k;
[off_sq_sample,off_sq] = Sample2D_Stratified(gI,N,u,v,u_cdf,v_cdf);
off_sq = off_sq';
...
while(converged == 0)    
   ...
   % hold on;
    condition(con,1)=i;
    condition(con,2) = abs(likhood/prevLikhood-1);
    con=con+1;
    %hold off;
    abs(likhood/prevLikhood-1)
...
while(converged == 0)    
     ...
     % [sq,pdf_sigma] =Pic_Sam(gI,1,cdf);
      [onli_sq_sample,sq] = Sample2D_test(gI,k,u,v,u_cdf,v_cdf); 
...
imshow(image);
save('imageK100M1000.mat','image');

ConvergeFig=figure(4);
plot(condition(:,1),condition(:,2),'r');
line(condition(:,1),condition(:,2),'Marker','o');

savefig(ConvergeFig,'ConvergeK100M1000.fig');
savefig(PointsFig,'PointsK100M1000.fig');
savefig(GaussFig,'GaussK100M1000.fig');
savefig(ResultFig,'ResultK100M1000.fig');
